...
module.exports.createThread = async (req, res, next) => {
  ...
  let { boardId, title, body } = JSON.parse(req.body.postData)
  
  // Check if boardId is provided and non-empty
  if (boardId && boardId.trim() === '') {
  return next(createError.BadRequest('boardId must not be empty'))
  }
  
  // Check if title and body are provided and non-empty
  if (!title || title.trim() === '') {
  return next(createError.BadRequest('Thread title must not be empty'))
  }
  
  if (!body || body.trim() === '') {
  return next(createError.BadRequest('Thread body must not be empty'))
  }
  ...
  boardId: boardId ? boardId.trim() : null,
  pined: false,
  closed: false,
  ...
  // Update board only if boardId is provided
  if (boardId) {
